...
function createUnixSocketProxy(unixAddr, cb) {
  const server = net.createServer((conn) => {
    const unixConn = net.createConnection(unixAddr);
    unixConn.on('error', () => {
      conn.end();
    });
    conn.pipe(unixConn);
...
function testExists(fileDesc) {
  if (fileDesc.file && fileDesc.file[0] === '/') {
    return fs.existsSync(fileDesc.file);
  } else {
    console.log(path.resolve(fileDesc.root, fileDesc.file));
    return fs.existsSync(path.resolve(fileDesc.root, fileDesc.file));
  }
  return false;
}
...
function createClient(args, options) {
  ...
  if (!testExists(file)) {
    throw new Error("File does not exist: " + JSON.stringify(file));
  }
  ...
  if (args.service) {
    const matcher = new RegExp(args.service, 'i');
...
function init(service, args, options) {
  ...
  const ev = loadEval(args);
  const creds = createCredentials(options);
  const client = new service.def(address, creds);
  ...
  if (ev && ev.length > 0) {
    ...
    new vm.Script(ev, {
      displayErrors: true
    }); // We only use this to get nice compile errors
    _eval(ev, scope);
