...
function createClient(args, options) {
  ...
  const cwd = (args.directory) ? args.directory : process.cwd();
  const filename = path.resolve(cwd, args.proto);

  if (!fs.existsSync(filename)) {
    throw new Error("File '" + filename + "' does not exist");
  }

  let protoDef = protoLoader.loadSync(filename);
  let parsed = grpc.loadPackageDefinition(protoDef);
...
function foundServiceClient(obj) {
  return Object.keys(obj).some(key => obj[key].service);
}
